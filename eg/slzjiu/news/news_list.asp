    ...
    <tr bgcolor="#FFFFFF" onMouseOver="this.bgColor='#CDE6FF'" onMouseOut="this.bgColor='#FFFFFF'">
     ...
     <td width="16%" height="30" align="center">
      <%
		...
		if show_hot=1 then
		 if rs("S_ok") then
		    response.Write("<a href="&url&"&action=s_ok&s_ok=0&id="&rs("id")&" onclick=""return confirm('是否取消推荐');""><font color=red>推荐</font></a>&nbsp;")
		 else
		    response.Write("<a href="&url&"&action=s_ok&s_ok=1&id="&rs("id")&" onclick=""return confirm('是否设置为推荐');""><font color=blue>推荐</font></a>&nbsp;")
		 end if
		end if
'		if s_pai=1 then
'			if rs("S_hot") then
