...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="tjnrym">
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="2" cellpadding="3" align="center">
     ...
     <%if show_img=1 then%>
     <tr bgcolor="<%=Color_0%>">
      <td height="20" align="right" valign="middle" bgcolor="<%=Color_0%>">信息图片:</td>
      <td height="20" valign="middle"><%=Upload_Init()%><%=Upload_Input("S_img",s_img)%></td>
     </tr>
     <%end if%>
	 <tr>
...
S_img= replace(trim(request.form("S_img")),"../","")
