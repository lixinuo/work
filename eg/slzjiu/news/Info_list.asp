...
case "save_add"
	...
	'Response.Write "<script Language=Javascript>alert('操作成功!');history.go(-1);< /script>"
	msg "操作成功","?action=add&id="&Request("id")&"&s_pai="&s_pai&"&show_del="&show_del&"&show_img="&show_img
     ...
     <tr valign="middle" bgcolor="#f3f3f3">
      ...
      <tr>
        <td class="lmtjdt">[<b><%=rs.pagecount%></b>/<%=page%>页] [共<%=totalfilm%>个]
          ...
          <%if show_del=1 then%>
          <input type="button" value=" 添加文章 " class="inputkkys"  name="buttonurl" onClick="location='?action=add&s_pai=<%=s_pai%>&show_del=<%=show_del%>&show_img=<%=show_img%>'">
          <%end if%>  </td>
      </tr>
    </table>	  </td>
     </tr>
...
set rs=db(strsql,2)
id=rs("id"):s_pai=rs("s_pai"):S_name=rs("S_name"):s_img=rs("s_img"):s_name1=rs("s_name1"):s_name2=rs("s_name2"):s_content=rs("s_content"):s_content1=rs("s_content1"):s_content2=rs("s_content2"):S_bt=rs("S_bt"):S_bt1=rs("S_bt1"):S_bt2=rs("S_bt2"):S_gjc=rs("S_gjc"):S_gjc1=rs("S_gjc1"):S_gjc2=rs("S_gjc2"):S_ms=rs("S_ms"):S_ms1=rs("S_ms1"):S_ms2=rs("S_ms2")
rs.close:set rs=nothing
else
...
%>
  ...
  <tr>
        ...
        <tr>
         <td width="9%" height="20" align="right" valign="middle" bgcolor="<%=Color_0%>">信息标题:</td>
         <td width="91%" height="20" valign="middle" bgcolor="<%=Color_0%>"><input name="S_name" type="text" class=inputkkys  size="50" value="<%=S_name%>"></td>
        </tr>
		 <%if instr(weblanguage,"1") then%>
		<tr>
         <td width="9%" height="20" align="right" valign="middle" bgcolor="<%=Color_0%>">信息标题(en):</td>
         <td height="20" valign="middle" bgcolor="<%=Color_0%>"><input name="s_name1" type="text" class=inputkkys  size="50" value="<%=s_name1%>"></td>
		</tr>
		<%end if%>
		<%if instr(weblanguage,"2") then%>
		<tr>
         <td width="9%" height="20" align="right" valign="middle">信息标题(日):</td>
         <td height="20" valign="middle" bgcolor="<%=Color_0%>"><input name="s_name2" type="text" class=inputkkys  size="50" maxlength="50" value="<%=s_name2%>"></td>
        </tr>
		<%end if%>
        <%if request("show_img")<>0 then%>
        <tr>
         <td width="9%" height="20" align="right" valign="middle" bgcolor="<%=Color_0%>">上传资料:</td>
         <td height="20" valign="middle" bgcolor="<%=Color_0%>"><%=Upload_Init()%><%=Upload_Input("S_img",s_img)%></td>
        </tr>
        <%end if%>
		<tr>
        ...
        <%end if%>
        <tr>
         <td width="9%" height="36" align="right" valign="bottom" bgcolor="<%=Color_0%>">信息内容:</td>
         <td height="36" valign="middle" bgcolor="<%=Color_0%>">
		 <%if webbjq=1 then%>
	  ...
	  <%end if%>
        ...
        <%if instr(weblanguage,"1") then%>
        <tr bgcolor="<%=Color_0%>">
         <td width="9%" height="20" align="right" valign="bottom" bgcolor="<%=Color_0%>">Content:</td>
	  ...
	  <%end if%>
        ...
        <%if instr(weblanguage,"2") then%>
        <tr bgcolor="<%=Color_0%>">
         <td width="9%" height="20" align="right" valign="bottom" bgcolor="<%=Color_0%>">日:</td>
         <td height="20" valign="middle" bgcolor="<%=Color_0%>">
		 <%if webbjq=1 then%>
