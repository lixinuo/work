...
if request("action")="orderby" then
  strIDs=request("strIDs")
	...
	strSQL="update A_class set s_order='"&orderby&"' Where id=" & strIDs & ""
	conn.execute strSQL
end if
if request("action")="del" then
  selectbookid=request("selectbookid")
	if selectbookid<>"" then
	strSQL="Delete From A_class Where id in (" & selectbookid & ")"
	conn.execute strSQL
	end if
end if
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="lmtj">
      <tr>
        <td class="lmtjdt">分类列表 <%if id=0 then%><%else%>*当前父类信息：<strong style="color:#F00;"><%=db_name("a_class",id)%><%end if%></strong></td>
      </tr>
</table>
...
strSQL ="select * from A_class where s_pai="&s_pai
if id<>0 then
...
%>
   <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="lmtj">
     ...
     <td height="30" colspan="6"><table width="90%" border="0" align="right" cellpadding="0" cellspacing="0">
       <tr>
        <td><%if session("show_add")=1 then%>
         ...
         <%end if%>
         <input type="button" name="backlist" class="inputkkys" onClick="location='class_list.asp';" value="返回">        </td>
        <td></td>
       </tr>
      </table></td>
    </tr>
    ...
    <tr>
     <td width="14%" height="30">&nbsp;分类ID号</td>
...
%>
    <tr bgcolor="#FFFFFF" onMouseOver="this.bgColor='#CDE6FF'" onMouseOut="this.bgColor='#FFFFFF'">
     ...
     <%	if session("show_next")=1 and rs("class_depth")<session("show_depth")+0 then%>
     <td width="18%" height="30" align="center"><%response.Write("<a href='class_list.asp?id="&rs("id")&"&s_pai="&s_pai&"'>进入下级分类("&db_s("select count(id) from A_class  where parent_id="&rs(0))&")</a>")%>     </td>
     <%end if%>
     <td height="30" align="center" > <A HREF="class_do.asp?id=<%=rs(0)%>&parent_id=<%=id%>&s_pai=<%=s_pai%>"><img src="../images/detail_off.gif" border=0></A></td>
...
sttSQL="select * from A_main,A_class where (A_main.Classid="&rs("id")&" and A_main.s_pai="&s_pai&") or (A_class.parent_id="&rs("id")&" and A_class.s_pai="&s_pai&") "
...
%>
    <tr>
     <td height="30" colspan="6" align="center"><%response.Write( fPageCount(rs,int_showNumberLink_,str_nonLinkColor_,toF_,toP10_,toP1_,toN1_,toN10_,toL_,showMorePageGo_Type_,cPageNo)  & vbcrlf )%>
      ...
      全选
      <input type="submit" name="Submit" value="删除所选" class="inputkkys" onClick="return test();">
      <%
			 end if
			end if
			%>     </td>
